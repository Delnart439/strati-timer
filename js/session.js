...
function getPuzIcon(puzzle) {
  ...
  if (puzzle==='FTO') return `<svg viewBox="0 0 20 20" width="18" height="18" xmlns="http://www.w3.org/2000/svg"><polygon points="1,1 19,1 10,10" fill="currentColor"/><polygon points="19,1 19,19 10,10" fill="currentColor" opacity="0.5"/><polygon points="19,19 1,19 10,10" fill="currentColor"/><polygon points="1,19 1,1 10,10" fill="currentColor" opacity="0.5"/></svg>`;
...
function generateShareImg(type, param) {
  ...
  const _sesAllCubes = [...new Set(ts.filter(t=>t.cubeName).map(t=>_friendlyCubeName(t.cubeName)).filter(Boolean))];
  const _sesHasStackmat = ts.some(t=>(t.manual||t.stackmat)&&!t.cubeName);
  const _sesHasKeyboard = ts.some(t=>!t.cubeName&&!t.manual&&!t.stackmat);
  const sessionDevices = [..._sesAllCubes.map(name=>({type:'cube',name})),...(_sesHasStackmat?[{type:'stackmat'}]:[]),...(_sesHasKeyboard?[{type:'keyboard'}]:[])];
